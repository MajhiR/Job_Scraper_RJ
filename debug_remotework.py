#!/usr/bin/env python
"""Diagnostic script to inspect RemoteWork.com HTML structure."""

import requests
from bs4 import BeautifulSoup

BASE_URL = "https://www.remote.work/jobs?search=machine+learning"
HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
}

print("=" * 80)
print("REMOTEWORK.COM DIAGNOSTIC SCRIPT")
print("=" * 80)
...
try:
    ...
    if response.status_code != 200:
        ...
        selectors = [
            ('div.job-item', 'div with job-item class'),
            ('li.job-item', 'li with job-item class'),
            ('div[class*="job"]', 'div with "job" in class'),
            ('article.job', 'article with job class'),
            ('div.listing', 'div with listing class'),
            ('div[data-job-id]', 'div with data-job-id'),
        ]
